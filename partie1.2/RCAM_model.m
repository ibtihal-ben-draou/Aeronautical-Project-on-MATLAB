...
depsda = 0.25;     %hange in downwash w.r.t. alpha (rad/fad)
...
al = -155.2; % Coefficient of alpha^1
a0 = 15.212; % Coefficient of alpha^0(different from RCAM docum
alpha_switch = 14.5* (pi/180); %alpha where lift slope goes from linear to non-li

%--1. CONTROL LIMITS/SATURATION-
%Not that these can alternately be enforced in Simulink
u1min = -25*pi/180;
u1max = 25*pi/180;
...
if alpha<=alpha_switch 
    ...
    CL_wb = a3*alpha^3+ a2*alpha^2 + al*alpha + a0;
end
...
%-----------5. AERODYNAMIC MOMENT COEFFICIENT ABOUT AC- Calculate the moments in Fb. Define eta, dcMdx and dCMdu 
etall = -1.4*beta;
...
dCMdu = [-0.6 0 0.22;
           ...
           0 0 -0.63];
%Now calculate CM = [C1; Cm; Cn] about Aerodynamic center in Fb 
